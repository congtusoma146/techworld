        ...
        stage('publish to ecr')
        {
            steps{
                ...
                withEnv(["AWS_ACCESS_KEY_ID=${env.AWS_ACCESS_KEY_ID}", "AWS_SECRET_ACCESS_KEY=${env.AWS_SECRET_ACCESS_KEY}", "AWS_DEFAULT_REGION=${env.AWS_DEFAULT_REGION}", "AWS_FORMAT=${env.AWS_FORMAT}"]) {
                    ...
                    /* groovylint-disable-next-line LineLength */
                    // push len task definition
                    sh "sed -i 's|techworld:.*|techworld:${env.BUILD_ID}|g' task_definition.json"
                    sh "aws ecs register-task-definition --family techworld-task --cli-input-json file://${env.WORKSPACE}/container.json"
                    // lay arn task definition moi nhat
                    def arn = aws ecs describe-task-definition --task-definition $(aws ecs list-task-definitions --status ACTIVE --sort DESC | jq -r '.taskDefinitionArns[0]')

                    //push lên ECS
                    /* groovylint-disable-next-line LineLength */
                    sh "aws ecs update-service --cluster techwrorld-cluster --service techworld-service --task-definition ${arn}" 
